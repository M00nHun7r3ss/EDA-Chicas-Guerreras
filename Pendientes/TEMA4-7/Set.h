...
public:
    ...
    // Complejidad O(n * log(n)), n=nelems.
    // Relacion de inclusion
    bool operator<=(const Set<T>& other) const{
        int i = 0;
        bool incluido = true; // inicialmente asumimos que todos los valores estaran incluidos.
        // mientras todos los valores vayan estando incluidos...
        while (i < nelems && incluido){ 
            // en el momento en el que un valor de this no este incluido en other...
            if (!other.contains(array[i])){ 
                // se para y no tienen relacion de inclusion.
				incluido = false;
            }
            i++;
        }
        ...
        // while() -> O(n)
        // contains() -> O(log(n))
        // resto de operaciones -> O(1)
        // Conclusion: while(O(log(n)), O(n) * O(log(n)), O(n * log(n)).
    }
