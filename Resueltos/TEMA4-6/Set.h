...
public:
    ...
    // Complejidad lineal. O(n), n=nelems. El tiempo de ejecucion es directamente proporcional al tamanio de la entrada.
    int getMax(){
        // le damos el primer valor.
        int max = array[0];

        // iteramos hasta el final buscando el maximo a partir del segundo valor.
    ...
    // Como remove es O(n) y getMax es O(n), O(n) + O(n) = O(n), tambien es complejidad lineal.
    void removeMax(){
        remove(getMax());
    }
    ...
    int getMin(){
        ...
        // iteramos hasta el final buscando el minimo a partir del segundo valor.
        for (int i = 1; i < nelems; ++i)
    ...
    // Como remove es O(n) y getMin es O(n), O(n) + O(n) = O(n), tambien es complejidad lineal.
    void removeMin(){
        remove(getMin());
    }
