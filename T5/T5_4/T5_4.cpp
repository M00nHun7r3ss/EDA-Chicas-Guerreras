...
public:
    void duplica() {
        ...
        while (act != nullptr) {
            ...
            //Creamos el nodo duplicado
            Nodo* nuevo = new Nodo(act->elem);

            //Lo insertamos justo despues del actual
            nuevo->sig = act->sig;
            act->sig = nuevo;

            //Si el actual era el ultimo, actualizamos ult
            if (act == this->ult) {
                this->ult = nuevo;
            }

            //Avanzamos al siguiente original y saltamos el duplicado
            act = nuevo->sig;
        }
